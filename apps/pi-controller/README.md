...
3. **Install the systemd unit:**
   ...
   The unit ships with two `ExecStart` lines, only one of which is active:

   - **Option A (default)** sources `~/.nvm/nvm.sh` before launching node, so
     it survives `nvm install` / `nvm use` swaps. Best for nvm setups.
   - **Option B (commented out)** is a hardcoded absolute path. Slightly
     faster startup, but you have to update it whenever you bump Node.

   If Node is installed system-wide (apt / NodeSource), comment Option A out
   and uncomment Option B with `ExecStart=/usr/bin/node /home/pi/.../dist/index.js`.
